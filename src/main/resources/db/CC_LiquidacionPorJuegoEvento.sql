CREATE PROC [dbo].[CC_LiquidacionPorJuegoEvento] @fecha_actual smalldatetime
AS
...
INNER JOIN CCorrientes.dbo.v_Operatoria o ON m.c_operatoria = o.c_operatoria
WHERE m.c_operacion in (SELECT distinct c_operacion as operaciones FROM CCorrientes.dbo.v_MacroLiquidacion
where CONVERT(DATE,f_macroliquidacion) = @fecha_actual and c_operatoria in ('DA', 'CA', 'CT', 'JU', 'LI', 'LL', 'LV')) and n_legajo = 723204
